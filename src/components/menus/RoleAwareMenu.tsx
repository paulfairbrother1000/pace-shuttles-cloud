...
function isSignedInFromCache(): boolean {
  try {
    ...
    if (!u) return false;
    // consider presence of any of these as "signed in"
    return Boolean(
      u.id || u.user_id || u.email || u.session || u.token || u.role || u.first_name
    );
  } catch {
    return false;
  }
...
function buildMenu(p: Profile | null): {
  ...
  burgerItems: { label: string; href: string }[]; // includes Chat/Support
} {
