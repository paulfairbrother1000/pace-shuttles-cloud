...
useEffect(() => {
  ...
  /* ---------- Occurrences (6 months rolling, honour min lead + season) ---------- */
  ...
  /* ---------- lookups ---------- */
  const pickupById = (id: string | null | undefined) =>
    pickups.find((p) => p.id === id) || null;
  const destById = (id: string | null | undefined) =>
  ...
  const vehicleTypeNameForRoute = (routeId: string): string => {
    const vs = assignments
      ...
      .map((a) =>
        vehicles.find((v) => v && v.id === a.vehicle_id && v.active !== false)
      )
  ...
  const boatsByRoute = useMemo(() => {
    const m = new Map<
      string,
      { vehicle_id: string; cap: number; preferred: boolean }[]
    >();
  ...
  const remainingSeatsByKey = useMemo(() => {
    ...
    for (const occ of occurrences) {
      ...
      if (!boats.length) {
        m.set(`${occ.route_id}_${occ.dateISO}`, 0);
        continue;
      }
      const parties =
        partiesByKey.get(`${occ.route_id}_${occ.dateISO}`) ?? [];
  ...
  useEffect(() => {
    if (filterDestinationId && !facetDestIds.has(filterDestinationId)) {
      setFilterDestinationId(null);
    }
  }, [facetDestIds, filterDestinationId]);

  useEffect(() => {
    if (filterPickupId && !facetPickupIds.has(filterPickupId)) {
      setFilterPickupId(null);
    }
  }, [facetPickupIds, filterPickupId]);

  useEffect(() => {
    if (filterTypeName && !facetTypeNames.has(filterTypeName)) {
