...
type RoleRow = { type_id: string; name: string }; // mapped from transport_type_role
type StaffRow = {
  id: string;
  operator_id: string;
  first_name: string;
...
export default function OperatorStaffPage() {
  ...
  /* Derived: operator locked + display name (fallback to lookup if operator_name missing) */
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // optional roles table (correct column names -> mapped to { type_id, name })
      const rolesRes = await sb
        .from("transport_type_role")
        .select("id, transport_type_id, role");
      if (!rolesRes.error && rolesRes.data) {
        const mapped: RoleRow[] = (rolesRes.data as any[]).map(r => ({
          type_id: r.transport_type_id,
          name: r.role,
        }));
        setRoles(mapped);
      }
    ...
    <div className="space-y-8">
      <header>
        ...
        <p className="text-neutral-600">
          {isOpAdmin
            ? <>Showing staff for <strong>{lockedOperatorName || psUser?.operator_id}</strong>.</>
