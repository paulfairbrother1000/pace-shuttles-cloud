...
export default function Client() {
  ...
  const rows: UiRow[] = useMemo(() => {
    ...
    for (const j of journeys) {
      ...
      if (isLocked) {
        ...
        for (const b of boats) {
          ...
          const mergedGroups = [...locked.groups, ...((prev?.orders ?? []).map((o) => o.size))].sort((a, b) => b - a);
    ...
    crewMinByJourney,
    operatorFilter,
    // lookup maps below are stable per deps above
  ]);
      ...
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-neutral-600 text-sm">
            ...
            you may keep accepting bookings; press <strong>Lock</strong> to persist an updated split.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            value={isOpAdminLocked ? (lockedOperatorId || "all") : operatorFilter}
            ...
            >
              ...
              <div className="p-4 flex flex-wrap items-center gap-3 border-b bg-neutral-50">
                ...
                <div className="ml-auto flex items-center gap-2">
                  ...
                  <span className="text-xs text-neutral-700">Paid: <strong>{row.totals.proj}</strong></span>
                  <span className="text-xs text-neutral-700">Locked: <strong>{row.totals.dbTotal}</strong></span>
                  <span className="text-xs text-neutral-700">Max: <strong>{row.totals.maxTotal}</strong></span>
                  <span className="text-xs text-neutral-700">Unassigned: <strong>{row.totals.unassigned}</strong></span>
