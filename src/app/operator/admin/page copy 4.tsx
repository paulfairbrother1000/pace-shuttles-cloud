...
type ManifestPaxRow = { first_name: string | null; last_name: string | null; is_lead: boolean };
...
export default function JourneyBoardsPage(): JSX.Element {
  ...
  const [manifestPax, setManifestPax] = React.useState<ManifestPaxRow[] | null>(null); // preferred
  ...
  async function openManifest(journeyId: string, title: string, subtitle: string) {
    ...
    setErr(null);
    try {
      // Preferred: names from journey_order_manifest view
      let filled = false;
      try {
        const { data: paxData, error: paxErr } = await supabase
          .from("journey_order_manifest")
          .select("first_name,last_name,is_lead")
          .eq("journey_id", journeyId)
          .order("is_lead", { ascending: false })
          .order("last_name", { ascending: true });

        if (!paxErr && paxData) {
          setManifestPax(paxData as ManifestPaxRow[]);
          filled = (paxData as ManifestPaxRow[]).length > 0;
        }
      } catch {
        // view missing / RLS — fall back
      }

      // Fallback: show bookings list
      if (!filled) {
        const { data, error } = await supabase
          .from("bookings")
      ...
      {manifestOpen && (
        <div className="fixed inset-0 z-40">
          ...
          <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-xl p-4 overflow-auto">
            ...
            ) : manifestPax && manifestPax.length > 0 ? (
              <ul className="mt-4 space-y-2">
                {manifestPax.map((p, idx) => {
                    ...
                    <li key={idx} className="rounded-md border p-2">
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          {p.is_lead ? (
                            <div className="inline-block mt-1 rounded-full bg-blue-50 px-2 py-0.5 text-[11px] text-blue-700">
                              Lead passenger
                            </div>
                          ) : null}
                        </div>
