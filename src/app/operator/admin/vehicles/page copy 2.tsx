...
type VehicleRow = {
  ...
  type_id: string | null;
  operator_id: string | null; // uuid in DB, string here
};
...
export default function OperatorAdminVehiclesPage() {
  ...
  const [operatorId, setOperatorId] = useState(""); // may be auto-locked from ps_user
  ...
  /* Load ps_user from localStorage (your app already sets this) */
  useEffect(() => {
    try {
  ...
  /* Access guard (Operator Admin or Site Admin) */
  const canAccess = !!(psUser?.operator_admin || psUser?.site_admin);
  ...
  /* Load lookups + vehicles; re-run when operatorId changes (key fix) */
  useEffect(() => {
    if (!canAccess) return; // wait for ps_user
    let off = false;
    (async () => {
      setLoading(true);
      ...
      // Vehicles (filter at source when operatorId is set)
      let vs;
      if (operatorId) {
        vs = await sb.from("vehicles").select("*")
          .eq("operator_id", operatorId)
          .order("created_at", { ascending: false });
      } else {
        // site admins / unlocked contexts
        vs = await sb.from("vehicles").select("*")
          .order("created_at", { ascending: false });
      }
  ...
  async function onSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (!editingId) {
        // CREATE (your existing API routes)
        const payload = {
  ...
  async function onRemove(row: VehicleRow) {
    ...
    const res = await fetch(`/api/admin/vehicles/${row.id}`, {
      method: "DELETE",
  ...
  const filtered = useMemo(() => {
    ...
    // If an operator is selected (or locked), limit the table to that operator's vehicles
  ...
  if (!canAccess) {
    return (
      <div className="p-6">
        <h1 className="text-xl font-semibold">Operator Admin</h1>
        <p className="text-neutral-600 mt-2">You don’t have access to this page.</p>
      </div>
    );
  }
      ...
      <header>
        ...
        <p className="text-neutral-600">
          {operatorLocked
            ...
            : <>Create, edit and delete vehicles. Images upload to Storage and transport type options are limited by the selected Operator.</>}
        </p>
      </header>
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow">
        <form onSubmit={onSave} className="space-y-5">
          <div className="grid md:grid-cols-3 gap-4">
            {/* Operator field: locked or selectable */}
            <div>
              <label className="block text-sm text-neutral-600 mb-1">Operator *</label>
              {operatorLocked ? (
                <div className="flex items-center gap-2">
                  <span className="inline-flex rounded-full bg-neutral-100 border px-3 py-2 text-sm">
