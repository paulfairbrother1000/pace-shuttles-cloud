...
export default function OperatorAdminVehiclesPage() {
  /* Safe Supabase client created at runtime */
  ...
  if (!sb) {
    return (
      <div className="p-6">
        <h1 className="text-2xl font-semibold">Operator Vehicles</h1>
        <p className="text-red-600 mt-2">
          Missing Supabase env vars. Add <code>NEXT_PUBLIC_SUPABASE_URL</code> and{" "}
          <code>NEXT_PUBLIC_SUPABASE_ANON_KEY</code> to <code>.env.local</code> and restart dev.
        </p>
      </div>
    );
  }

  /* ---------- Auth session (important for RLS) ---------- */
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const listener = sb.auth.onAuthStateChange((_e, s) => {
  ...
  const [operatorId, setOperatorId] = useState(""); // locked from ps_user if operator_admin
  ...
  const lockedOperatorName =
    ...
    (operatorLocked && operators.find(o => o.id === psUser!.operator_id!)?.name) ||
    "";
  ...
  useEffect(() => {
    let off = false;
    (async () => {
      const entries = await Promise.all(rows.map(async r => [r.id, await signedUrl(r.picture_url || null)] as const));
      if (!off) setPicUrlMap(Object.fromEntries(entries));
    })();
    return () => { off = true; };
  }, [rows]);

  /* ---------- Load lookups + vehicles (after we know session & operator) ---------- */
  useEffect(() => {
    // Wait until we know whether a session exists (null means "no session")
    ...
    (async () => {
      ...
      // Vehicles — filter at source if operatorId is known
      const vs = operatorId
        ? await sb.from("vehicles").select("*").eq("operator_id", operatorId).order("created_at", { ascending: false })
        : await sb.from("vehicles").select("*").order("created_at", { ascending: false });

      if (off) return;

      if (ops.error || jts.error || rels.error || vs.error) {
        console.error("Load errors:", { ops: ops.error, jts: jts.error, rels: rels.error, vs: vs.error });
        setMsg(ops.error?.message || jts.error?.message || rels.error?.message || vs.error?.message || "Load failed");
      }

      // Debug: what came back
      console.log("vehicles.query", {
        operatorId,
        sessionUserId,
        rows: (vs.data as any[])?.length ?? 0,
        error: vs.error,
      });
    ...
    return () => { off = true; };
  }, [sessionUserId, operatorId, sb]);

  async function reloadRows() {
    const query = operatorId
      ? sb.from("vehicles").select("*").eq("operator_id", operatorId).order("created_at", { ascending: false })
  ...
  function resetForm() {
    setEditingId(null);
    setOperatorId(operatorLocked ? (psUser?.operator_id || "") : "");
    setTypeId("");
    setName("");
    setMinSeats("");
    setMaxSeats("");
  ...
  async function loadOne(id: string) {
    ...
    if (error || !data) { setMsg(error?.message ?? "Could not load vehicle."); return; }
    const v = data as VehicleRow;
    setEditingId(id);
    setOperatorId(v.operator_id ?? (operatorLocked ? (psUser?.operator_id || "") : ""));
    setTypeId(v.type_id ?? "");
    setName(v.name ?? "");
  ...
  async function uploadPictureIfAny(vehicleId: string): Promise<string | null> {
    ...
    const { error } = await sb.storage.from("images")
  ...
  async function onSave(e: React.FormEvent) {
    ...
    try {
      ...
      const minS = toInt(minSeats), maxS = toInt(maxSeats), minV = toFloat(minValue);
    ...
    <div className="space-y-8">
      <header>
        ...
        <p className="text-neutral-600">
          {operatorLocked
            ? <>Operator is locked to <strong>{lockedOperatorName || psUser?.operator_id}</strong> based on your login.</>
            : <>Create, edit and delete vehicles.</>}
        </p>
        <p className="text-xs text-neutral-500 mt-1">
          Debug: session user = {sessionUserId ?? "none"} • operatorId = {operatorId || "—"} • rows = {rows.length}
        </p>
      </header>
      ...
      <section className="space-y-3">
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white overflow-hidden shadow">
          {loading ? (
            ...
            <div className="p-4">No vehicles yet for this operator.</div>
          ) : !operatorId ? (
            <div className="p-4">Choose an Operator to list vehicles.</div>
          ) : (
