...
/* Strict UI quote (must have token; price is per-seat all-in, in pounds as an integer for display) */
type UiQuote = { displayPounds: number; token: string };
...
export default function HomePage() {
  ...
  // (We still keep refs around, but we no longer gate fetching on visibility.)
  const rowRefs = useRef<Record<string, HTMLTableRowElement | null>>({});

  function parseStrictQuote(json: any): UiQuote | null {
    const token = json?.token ?? json?.quoteToken ?? json?.quote?.token ?? null;
    const perSeat = json?.perSeatAllInC ?? json?.quote?.perSeatAllInC ?? json?.allInC ?? json?.quote?.allInC ?? null;
    const perSeatNum = Number(perSeat);
    if (!token || !Number.isFinite(perSeatNum)) return null;
    return { displayPounds: Math.ceil(perSeatNum), token: String(token) };
  }

  async function fetchQuote(routeId: string, dateISO: string, qty: number): Promise<{ q: UiQuote | null; err: string | null }> {
    const tries: Array<() => Promise<Response>> = [
      () => fetch(`/api/quote?routeId=${encodeURIComponent(routeId)}&date=${encodeURIComponent(dateISO)}&qty=${qty}`, { cache: "no-store" }),
      () => fetch(`/api/quote?route_id=${encodeURIComponent(routeId)}&date=${encodeURIComponent(dateISO)}&qty=${qty}`, { cache: "no-store" }),
      () => fetch(`/api/quote`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ routeId, date: dateISO, qty }) }),
      () => fetch(`/api/quote`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ route_id: routeId, date: dateISO, qty }) }),
    ];
    ...
    for (const go of tries) {
      try {
        ...
        const parsed = parseStrictQuote(json);
        if (parsed) return { q: parsed, err: null };
        lastErr = "bad-payload";
      } catch { lastErr = "network"; }
    }
    return { q: null, err: lastErr ?? "fail" };
  }

  // 🔧 Fetch prices immediately for all rows (max 10) and whenever seat selections change.
  useEffect(() => {
    let off = false;
    (async () => {
      if (!rows.length) { setQuotesByRow({}); setQuoteErrByRow({}); return; }

      for (const r of rows) {
        if (off) return;
        // only fetch if we don't already have a quote for this row
        if (quotesByRow[r.key] == null) {
          const qty = seatSelections[r.key] ?? DEFAULT_SEATS;
          const { q, err } = await fetchQuote(r.route.id, r.dateISO, qty);
          if (off) return;
          setQuotesByRow((prev) => ({ ...prev, [r.key]: q }));
          setQuoteErrByRow((prev) => ({ ...prev, [r.key]: err }));
        }
      }
    })();
    return () => { off = true; };
  }, [rows, seatSelections]); // <- runs on initial render and whenever seat choice changes

  const handleSeatChange = async (rowKey: string, n: number) => {
    setSeatSelections((prev) => ({ ...prev, [rowKey]: n }));
    const row = rows.find((r) => r.key === rowKey);
    if (row) {
      const { q, err } = await fetchQuote(row.route.id, row.dateISO, n);
      setQuotesByRow((prev) => ({ ...prev, [rowKey]: q }));
      setQuoteErrByRow((prev) => ({ ...prev, [rowKey]: err }));
    }
  };

  // SOFT CONTINUE: Insert even if price hasn't arrived yet; redirect with qid
// SOFT CONTINUE: Insert even if price hasn't arrived yet; redirect with qid
const handleContinue = async (rowKey: string, routeId: string) => {
  ...
  try {
    // 1) Create/record the intent (works for guests)
    const { data, error } = await supabase
      ...
      .insert({
        ...
        seats,
        per_seat_all_in: q?.displayPounds ?? null,
        currency: "GBP",
    ...
    // 2) If not signed in yet, send them to login first with a next param
    ...
    // 3) Already signed in → straight to checkout
  ...
  // Use filteredOccurrences so the calendar reflects the 25-hour rule and other filters
  const namesByDate = useMemo(() => {
    ...
    const nameOf = (r: RouteRow) => {
      ...
      return `${pu} → ${de}`; // full name for calendar
    };
    ...
    <div className="space-y-8 px-4 py-6 mx-auto max-w-[1120px]">
      <header className="space-y-2">
        ...
        <p className="text-neutral-600">
          Choose your country, filter, pick a date — then continue to summary.
        </p>
      </header>
        ...
        <section className="grid md:grid-cols-3 gap-4">
          {countries.map((c) => (
            <button
              ...
              onClick={() => { setCountryId(c.id); setActivePane("date"); setFilterDateISO(null); setFilterDestinationId(null); setFilterPickupId(null); setFilterTypeName(null); setCalCursor(startOfMonth(new Date())); }}
            >
              {c.picture_url ? (
          ...
          <section className="rounded-2xl border border-neutral-200 bg-white p-4 shadow space-y-4">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-1 space-y-1">
                          {/* full route names, no truncation */}
                          {names.map((n, idx) => (
            ...
            {activePane === "destination" && (
              <TilePicker
                ...
                items={destinations.map((d) => ({ id: d.id, name: d.name, description: d.description ?? "", image: d.picture_url ?? undefined }))}
            ...
            {activePane === "pickup" && (
              <TilePicker
                ...
                items={pickups.map((p) => ({ id: p.id, name: p.name, description: p.description ?? "", image: p.picture_url ?? undefined }))}
            ...
            {activePane === "type" && (
              <TilePicker
                ...
                items={transportTypeRows.filter((t) => t.is_active !== false).map((t) => ({
                  id: t.name, name: t.name, description: t.description ?? "", image: t.picture_url ?? undefined,
                }))}
              ...
              <table className="w-full">
                ...
                <tbody>
                  {rows.map((r) => {
                    ...
                    const priceDisplay = hasLivePrice ? q!.displayPounds : 0;
                    ...
                    return (
                      <tr key={r.key} data-rowkey={r.key} ref={(el) => { rowRefs.current[r.key] = el }} className="border-t align-top">
                        ...
                        <td className="p-3">
                          <select className="border rounded-lg px-2 py-1" value={selected} onChange={(e) => handleSeatChange(r.key, parseInt(e.target.value))}>
                        ...
                        <td className="p-3">
                          <button
                            className={`px-3 py-2 rounded-lg text-white hover:opacity-90 transition ${
                              hasLivePrice ? "bg-blue-600 hover:bg-blue-700" : "bg-neutral-400"
                            }`}
                            title={hasLivePrice ? "Continue" : "Continue (price will be confirmed on next step)"}
