...
type UiQuote = { displayPounds: number; token: string; availability?: "available" | "sold_out" };
...
/** Single clean GET using snake_case (matches server) */
...
export default function HomePage() {
    ...
    const run = async () => {
      await Promise.all(
        rows.map(async (r) => {
          ...
          try {
            const json = await fetchQuoteOnce(r.route.id, r.dateISO, qty, ac.signal);
            ...
            const per = Number(json.perSeatAllInC);
            setQuotesByRow((p) => ({
              ...p,
              [r.key]: {
                displayPounds: Math.ceil(per),
                token: json.token,
  ...
  const handleSeatChange = async (rowKey: string, n: number) => {
    ...
    const ac = new AbortController();
    try {
      const json = await fetchQuoteOnce(row.route.id, row.dateISO, n, ac.signal);
      if ("error_code" in json) {
        const extra =
          json.step || json.details
            ? ` (${json.step ?? ""}${json.step && json.details ? ": " : ""}${json.details ?? ""})`
                    ...
                    <td className="p-3">
                      <button
                        className={`px-3 py-2 rounded-lg text-white hover:opacity-90 transition ${
                          isSoldOut ? "bg-neutral-400" : hasLivePrice ? "bg-blue-600 hover:bg-blue-700" : "bg-neutral-400"
                        }`}
                        title={
                          isSoldOut
                            ? "Sold out"
