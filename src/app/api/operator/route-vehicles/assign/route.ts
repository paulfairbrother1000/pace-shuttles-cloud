...
export async function POST(req: NextRequest) {
  try {
    const { routeId, vehicleId, preferred = false } = await req.json();
    ...
    // if operator-scoped, enforce ownership
    ...
    const up = await admin
      .from("route_vehicle_assignments")
      .upsert(
        [{ route_id: routeId, vehicle_id: vehicleId, is_active: true, preferred: !!preferred }],
