 ...
 * This prevents a user replying to someone else's ticket by guessing the ID.
 */
...
export async function POST(
  ...
  try {
    ...
    // Security: ensure this ticket belongs to the logged-in user
    ...
    if (!articleRes.ok) {
      return NextResponse.json(
        { ok: false, error: await articleRes.text() },
        { status: 502 }
      );
    }

    /**
     * 4) If ticket was Resolved (pending close), reopen it
     */
    if (status === "resolved") {
