...
function baseSeatPrice(minvalue: number, minseats: number) {
  const ms = Math.max(1, Number(minseats));
  const mv = Math.max(0, Number(minvalue));
  return Math.ceil(mv / ms);
}

function splitAllIn(baseSeat: number, taxRate: number, feesRate: number) {
  const tax = r2(baseSeat * taxRate);
  const fees = r2((baseSeat + tax) * feesRate);
  const allIn = r2(baseSeat + tax + fees);
  return { base: baseSeat, tax, fees, allIn };
}
...
function fail(step: string, error: any, diag: boolean) {
  const msg = typeof error?.message === "string" ? error.message : String(error);
  if (diag) {
    return ok({ error_code: "internal_error", step, details: msg }, 200);
  }
...
/** Ensure a journey for (routeId, dateISO). Try RPC; fall back to day-range lookup. */
async function ensureJourneyId(routeId: string, dateISO: string, diag = false) {
  const db = sbAdmin();
  // RPC first
  try {
    const { data, error } = await db.rpc("ps_ensure_journey", {
      p_route_id: routeId,
      p_day: dateISO,
    });
...
/** Booked seats (excluding cancelled) – total + by vehicle. */
...
/** Load active vehicles for the route from either `vehicles` or `transport_types`. */
async function loadAssignedVehiclesFlexible(routeId: string) {
  ...
  const active = (assigns || []).filter((r: any) => r?.is_active === true);
...
async function handleQuote(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      return fail("ensureJourneyId", e, diag);
    }
    if (!journeyId) return ok({ availability: "no_journey", error_code: "no_journey" });
    ...
    try {
      ...
      return fail("loadAssignedVehiclesFlexible", e, diag);
    }
    if (!vRes.items.length) {
      return ok(
        diag
          ? { availability: "no_vehicles", error_code: "no_vehicles", source: vRes.source, note: "no active assignments or craft missing" }
          : { availability: "no_vehicles", error_code: "no_vehicles" }
      );
    }
    const vehicles = decorateAndSort(vRes.items);
    ...
    try {
      ...
      return fail("getSoldSeats", e, diag);
    }
    ...
    if (remainingOverall <= 0) return ok({ availability: "sold_out" });
    ...
    type Candidate = { v: typeof vehicles[number]; price: number; remaining: number; discountActive: boolean };
    ...
    for (const v of vehicles) {
      ...
      const base = (v as any).base_price as number;
      const discountActive = openInfo[v.id]?.discountActive ?? false;
      const disc = asFraction(v.maxseatdiscount ?? 0);
      const effectiveBase = discountActive ? Math.ceil(base * (1 - disc)) : base;
      const { allIn } = splitAllIn(effectiveBase, taxRate, feesRate);

      candidates.push({ v: v as any, price: allIn, remaining, discountActive });
    }

    if (!candidates.length) {
      return ok({ availability: "insufficient_capacity_for_party", error_code: "insufficient_capacity_for_party" });
    }
    ...
    const base = chosen.discountActive
      ? Math.ceil((chosen.v as any).base_price * (1 - asFraction(chosen.v.maxseatdiscount ?? 0)))
      : (chosen.v as any).base_price;

    const { tax, fees, allIn } = splitAllIn(base, taxRate, feesRate);
    ...
    const payload: QuotePayloadV1 = {
      ...
      total_cents: Math.round(allIn * 100) * qty,
      currency: "GBP",
      iat: now,
      exp: now + 15 * 60,
    };
    ...
    try {
      ...
      return fail("signQuoteLocal", e, diag);
    }

    const resp: any = {
      availability: "available",
      qty,
      base_cents: payload.base_cents,
    ...
    if (diag) resp._diag = { source: vRes.source, totalSold, byVehicle, taxRate, feesRate };

    return ok(resp);
  } catch (e) {
    return fail("top-level", e, diag);
  }
}
