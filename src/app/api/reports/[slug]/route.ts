    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      cookies: {
        ...
        set(name: string, value: string, options?: any) {
          // Next will append Set-Cookie on the response automatically
          cookieStore.set(name, value, options as any);
        },
        remove(name: string, options?: any) {
          // remove via expired cookie
...
function mapSlugToFn(slug: string) {
  switch (slug) {
    case "revenue_by_route_date": return "rpt_revenue_by_route_date_v3"; // <-- new
...
export async function GET(req: NextRequest, { params }: { params: { slug: string } }) {
  try {
    ...
    const fn = mapSlugToFn(params.slug);
    if (!fn) return NextResponse.json({ error: `Unknown report slug: ${params.slug}` }, { status: 404 });
