...
type VehicleRow = {
  ...
  type_id: string | null;      // journey_types.id as string (align to your schema)
  operator_id: string | null;  // uuid in DB, typed as string here
};
...
export default function VehiclesPage() {
  /* Signed-in user (to lock/drive operator filtering) */
  const [me, setMe] = useState<PsUser | null>(null);
  ...
  const [operatorId, setOperatorId] = useState(""); // source of truth for filter + form
  ...
  /* 1) Load current user (so we get operator_id & role flags) */
  useEffect(() => {
    let off = false;
    (async () => {
      // requires users_select_self RLS policy
      const { data, error } = await sb
        .from("users")
        .select("id, site_admin, operator_admin, operator_id")
        .single();
      if (off) return;
      if (!error && data) {
        const u = data as PsUser;
        setMe(u);
        // If operator admin, auto-lock filter to their operator
  ...
  /* 2) Load lookups + vehicles (server-side filtered by operatorId when present) */
  useEffect(() => {
    let off = false;
    (async () => {
      setLoading(true);
      ...
      if (operatorId) {
        vs = await sb
          .from("vehicles")
          .select("*")
          .eq("operator_id", operatorId) // << server-side filter for operator admin
          .order("created_at", { ascending: false });
      } else {
        // site admins / initial state can see all (RLS governs)
        vs = await sb.from("vehicles").select("*").order("created_at", { ascending: false });
      }
  ...
  function resetForm() {
    setEditingId(null);
    // keep operatorId as-is (especially for operator admins)
    setTypeId("");
    setName("");
    setMinSeats("");
    setMaxSeats("");
  ...
  async function loadOne(id: string) {
    ...
    setEditingId(id);
    setOperatorId(v.operator_id ?? ""); // keep aligned with row (site admin may edit others)
    setTypeId(v.type_id ?? "");
    setName(v.name ?? "");
  ...
  /* ---------- TABLE FILTER (text search only; operator filter is server-side) ---------- */
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div>
              ...
              <select
                ...
                disabled={!!(me?.operator_admin && !me?.site_admin)} // lock for operator admins
              >
                <option value="">— Select —</option>
                {operators.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
              </select>
              {me?.operator_admin && !me?.site_admin && (
                <p className="text-xs text-neutral-500 mt-1">Locked to your operator.</p>
              )}
            </div>
            <div>
