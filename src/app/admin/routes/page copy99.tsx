...
export default function RoutesPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const [c, pu, de, r] = await Promise.all([
      ...
      if (c.error || pu.error || de.error || r.error) {
        console.error("Load errors:", { c: c.error, pu: pu.error, de: de.error, r: r.error });
        setMsg(c.error?.message || pu.error?.message || de.error?.message || r.error?.message || "Load failed");
      }
  ...
  async function loadOne(id: string) {
    ...
    setBasePrice((data.base_price_gbp ?? "").toString());
  ...
  async function onSave(e: React.FormEvent) {
    e.preventDefault(); // <-- critical: avoids posting the form to /admin/routes (page)
    try {
      setMsg(null);
      ...
      const payload = {
        ...
        base_price_gbp: toNum(basePrice),
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow">
        <form onSubmit={onSave} className="space-y-5">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm text-neutral-600 mb-1">Base Price (GBP)</label>
              <input className="w-full border rounded-lg px-3 py-2" inputMode="decimal" value={basePrice} onChange={(e) => setBasePrice(e.target.value)} />
            </div>
            <div>
          ...
          <div className="flex items-center gap-2">
            <button type="submit" disabled={saving || !countryId || !pickupId || !destinationId}
      ...
      <section className="space-y-3">
        <div className="flex gap-2">
          <input className="border rounded-lg px-3 py-2" placeholder="Search routes…"
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        {thumbPickup(r.pickup_id)
                          ? <img src={thumbPickup(r.pickup_id)!} className="h-10 w-16 object-cover rounded border" alt="pickup" />
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        {thumbDest(r.destination_id)
                          ? <img src={thumbDest(r.destination_id)!} className="h-10 w-16 object-cover rounded border" alt="destination" />
                          : <div className="h-10 w-16 rounded border bg-neutral-100" />}
