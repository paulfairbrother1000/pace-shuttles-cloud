...
type Row = {
  ...
  // New/linked fields (match your schema)
...
export default function RoutesPage() {
  ...
  async function onSave(e: React.FormEvent) {
    ...
    try {
      ...
      // Require journeyTypeId only when creating a new record
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={
                ...
                !pickupId ||
                !destinationId ||
                (!editingId && !journeyTypeId) // only required for create
              }
...
/* Local helpers used inside JSX */
function nameFor(id: string | null | undefined, list: { id: string; name: string }[]) {
  return (id && list.find((x) => x.id === id)?.name) || "—";
}
function thumbPickup(id: string | null | undefined) {
  return (id && (window as any)?.pickups?.find?.((p: any) => p.id === id)?.picture_url) || null;
}
function thumbDest(id: string | null | undefined) {
  return (id && (window as any)?.destinations?.find?.((d: any) => d.id === id)?.picture_url) || null;
}
