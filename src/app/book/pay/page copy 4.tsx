...
export default function PayPage(): JSX.Element {
  ...
  async function handlePayNow() {
    ...
    // Basic checks for the signed quote contract
    ...
    // Build passengers array with exactly one is_lead=true
    const passengers = [
    ...
    setSubmitting(true);
    try {
      // NOTE: backend will pick the best vehicle if vehicleId is omitted.
      // We pass acceptUpdatedPrice=true to avoid a confirm round-trip here.
      const res = await fetch("/api/client/book", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          // quote context (useful server-side for auditing)
          qid,
          quoteToken: token,
          perSeatAllIn: allInC,
          currency: ccy,
          ...
          // booking intent
          routeId,
          // vehicleId: undefined,           // let server choose from live quotes
          seats: qty,
          departISO: `${dateISO}T12:00:00Z`, // day-only -> midday UTC; server may override
          acceptUpdatedPrice: true,

          // lead contact (also mirrored to orders.lead_* by API)
          lead: {
          ...
          // passengers persisted to order_passengers
      ...
      // Server may respond 409 with { currentPrice } if it wants user confirmation;
      // for now we send acceptUpdatedPrice=true, so treat any 409 as an error message.
      if (res.status === 409) {
        setErr(
          json?.currentPrice
            ? `Price updated to ${gbp(Number(json.currentPrice))}. Please go back and retry.`
            : "Price changed. Please go back and retry."
        );
        setSubmitting(false);
        return;
      }
